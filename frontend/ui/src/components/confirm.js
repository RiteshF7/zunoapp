...
export function customConfirm(title, message, confirmText = 'Confirm', isDanger = false) {
  return new Promise(resolve => {
    ...
    document.getElementById('confirm-content').innerHTML = `
      ...
      <p class="text-muted-foreground text-sm mb-6">${message}</p>
      <div class="flex gap-3">
        ...
        <button id="confirm-ok" class="flex-1 py-3 rounded-xl text-sm font-semibold transition-colors active:scale-[0.97] ${isDanger ? 'bg-danger hover:bg-red-600 text-white' : 'bg-primary hover:bg-primary/90 text-primary-foreground'}">${confirmText}</button>
      </div>`;
