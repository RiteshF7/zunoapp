      ...
      <section class="bg-card rounded-md p-5 mb-4 shadow-sm border border-border" aria-label="Account info">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input id="p-name" value="${esc(p.display_name || '')}" class="w-full bg-bg border border-border rounded-md px-4 py-3 text-sm text-heading focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent/30" />
          </div>
          <div>
            <label for="p-avatar" class="text-xs text-muted-foreground font-medium mb-1.5 block">Avatar URL</label>
            <input id="p-avatar" value="${esc(p.avatar_url || '')}" class="w-full bg-bg border border-border rounded-md px-4 py-3 text-sm text-heading focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent/30" />
          </div>
      ...
      <section class="bg-card rounded-md p-5 mb-4 shadow-sm border border-border" aria-label="Preferences">
        ...
        <div class="mb-4">
          ...
          <div class="flex gap-2">
            <button onclick="updateFeedPref('usersaved')" class="flex-1 py-2.5 rounded-md text-sm font-medium transition-all duration-200 ${pref.feed_type === 'usersaved' ? 'bg-primary text-primary-foreground border-primary/30' : 'bg-card border border-border text-foreground'} active:scale-[0.97]">My Saved</button>
            <button onclick="updateFeedPref('suggestedcontent')" class="flex-1 py-2.5 rounded-md text-sm font-medium transition-all duration-200 ${pref.feed_type === 'suggestedcontent' ? 'bg-primary text-primary-foreground border-primary/30' : 'bg-card border border-border text-foreground'} active:scale-[0.97]">Suggested</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="flex gap-2">
            ${['light', 'dark', 'system'].map(t => `
              <button onclick="applyTheme('${t}');renderProfile(document.getElementById('page'))" class="flex-1 py-2.5 rounded-md text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1.5 ${theme === t ? 'bg-primary text-primary-foreground border-primary/30' : 'bg-card border border-border text-foreground'} active:scale-[0.97]">
